#!/usr/bin/env python3
"""
lmdb_inspect_linux.py - An interactive tool to inspect and verify the
contents of a CS2 data LMDB. (Linux Version)
...
# --- Linux-specific key press detection ---
import tty, termios
def getch():
    """Gets a single character from standard input on Linux/macOS."""
    fd = sys.stdin.fileno()
...
def debug_interactive_search(env):
    ...
    try:
        while True:
            ...
            char_bytes = getch()
            if char_bytes in (b'\x03', b'\x04'): break # Ctrl+C or Ctrl+D
            elif char_bytes == b'\x7f': # Backspace
                search_query = search_query[:-1]
            else:
                try:
                    char = char_bytes.decode('utf-8')
                    if char.isprintable(): search_query += char
                except UnicodeDecodeError: pass # Ignore non-printable chars
...
def main():
    ...
    while True:
        ...
        if frame is None:
            ...
            if run_mode_on:
                run_mode_on = False
                print("Run mode stopped: No data for tick.")
        ...
        if key == ord('r'): run_mode_on = not run_mode_on; os.system('clear'); print(f"Run mode toggled {'ON' if run_mode_on else 'OFF'}"); continue
        if run_mode_on:
