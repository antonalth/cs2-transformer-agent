...
(Headless version for video output)
...
- Renders all 5 player POVs in a compact **3x2 grid** (one empty slot is used
  to show global game-state labels — ground-truth vs prediction).
- Instead of interactive display, this script saves the visualization of one or
  more random samples to MP4 video files in a specified output folder.
...
def compose_grid(state: ViewerState) -> np.ndarray:
    ...
    # Footer bar with status
    bar_h = 28
    bar = np.zeros((bar_h, grid.shape[1], 3), np.uint8)
    txt = f"tick {state.rec.start_tick_win + f*TICKS_PER_FRAME}  |  frame {f+1}/{T}"
...
def main():
    p = argparse.ArgumentParser(description="CS2 ARP viewer (no DALI). This version runs headlessly and saves outputs to video files.")
    ...
    p.add_argument("--out-folder", required=True, help="Folder to save output MP4 videos.")
    p.add_argument("--samples", type=int, default=1, help="Number of random T-frame windows to sample and record.")
    ...
    # Function to select a sample and run the model on it
    def new_random_sample():
        tr = random.choice(state.rounds)
        state.rec = sample_random_window(tr, args.T)
        print(f"Sampling round {state.rec.round_num}, team {state.rec.team}, start_tick {state.rec.start_tick_win}...")
        run_model_on_record(state)

    # Main processing loop
    for i in range(args.samples):
        print(f"\n--- Processing sample {i + 1} of {args.samples} ---")
        new_random_sample()

        if state.rec is None or state.frames_5x is None:
            print("Failed to load sample data. Skipping.")
            continue

        # Determine video properties from the first composed frame
        state.t_idx = 0
        canvas = compose_grid(state)
        height, width, _ = canvas.shape
        
        # Set up video writer
        out_filename = f"{args.demoname}_round{state.rec.round_num:03d}_{state.rec.team}_tick{state.rec.start_tick_win}.mp4"
        out_path = os.path.join(args.out_folder, out_filename)
        # Use a moderate FPS for readability of overlays
        output_fps = 10.0
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        video_writer = cv2.VideoWriter(out_path, fourcc, output_fps, (width, height))

        print(f"Writing {state.rec.T_frames} frames to {out_path}...")

        # Loop through all frames of the sample and write to video
        for f_idx in range(state.rec.T_frames):
            state.t_idx = f_idx
            frame_canvas = compose_grid(state)
            video_writer.write(frame_canvas)

        video_writer.release()
        print(f"Successfully saved video for sample {i + 1}.")
