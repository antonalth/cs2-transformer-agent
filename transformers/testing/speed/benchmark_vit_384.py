...
# Model maps for different resolutions
MODEL_MAP_224 = {
...
MODEL_MAP_384 = {
    ...
    "huge": "google/vit-huge-patch14-224-in21k" # Note: No official 'huge-384' from google, uses 224 as fallback
}
...
def benchmark_tensorrt(engine_path, dummy_input, num_labels):
    """Benchmarks the optimized TensorRT engine with dynamic output buffer."""
    ...
    with engine.create_execution_context() as context:
        ...
        # DYNAMIC OUTPUT BUFFER - using num_labels passed from main
...
def build_tensorrt_engine(onnx_path, engine_path, use_fp16, batch_size):
    """Builds a TensorRT engine with an optimization profile."""
    TRT_LOGGER = trt.Logger(trt.Logger.WARNING)
...
def main():
    ...
    parser.add_argument('--res384', action='store_true', help="Use 384x384 resolution models instead of 224x224.")
    ...
    # --- SELECT MODEL MAP BASED ON RESOLUTION ---
    resolution = 384 if args.res384 else 224
    model_map = MODEL_MAP_384 if args.res384 else MODEL_MAP_224
    ...
    print(f"Model: {model_name}, Resolution: {resolution}, Precision: {precision}, Batch Size: {args.batch_size}, TensorRT: {args.tensorrt}")
    print("-------------------------------")
    ...
    # Input size is now dynamically determined by the loaded model's config
    ...
    if args.tensorrt:
        # --- FILENAMES NOW INCLUDE RESOLUTION ---
        onnx_filename = f"vit-{model_size}-{resolution}-{precision}.onnx"
        engine_filename = f"vit-{model_size}-{resolution}-{precision}.engine"
        ...
        # --- PASS DYNAMIC NUM LABELS TO BENCHMARK FUNCTION ---
        tensorrt_fps = benchmark_tensorrt(engine_filename, dummy_input, model.config.num_labels)

    print("\n\n--- BENCHMARK SUMMARY ---")
    print(f"Model: {model_name} @ {resolution}x{resolution}")
    print(f"Precision: {precision}, Batch Size: {args.batch_size}")
