...
INITIAL_MAP_SIZE = 2 * 1024**3
MAP_RESIZE_INCREMENT = 1 * 1024**3
...
def init_worker(shm_cache_name, shm_cache_size, lmdb_path, lock, gs_dtype, pi_dtype, rounds_info, recordings_map, audio_params):
...
def process_round_perspective(task_args):
    ...
    try:
        ...
        for tick in range(start_tick, end_tick + 1, TICKS_PER_FRAME):
            ...
            gs[0]['tick'] = tick
            gs[0]['team_alive'], gs[0]['enemy_alive'] = team_alive, enemy_alive
            ...
            for rec in round_data:
                ...
                if not is_alive or tick >= rec['stoptick']:
                    continue
                ...
                lk = f"{pn}:{tick}"
                td = merge_tick_data(player_data_cache.get(lk), player_data_cache.get(f"{pn}:{tick+1}"))
                ...
                player_tuple = (pi, None, mel_spectrogram)
                pdl_unsorted.append((p_idx, player_tuple))
            ...
            try:
                with env.begin(write=True) as txn:
                    ...
                    if available < batch_size:
                        grow_by = max(batch_size * 1.5, MAP_RESIZE_INCREMENT)
                        psize = stats['psize']
                        new_size = info['map_size'] + (((int(grow_by) + psize - 1) // psize) * psize)
                        # Must commit transaction before resizing
                        txn.commit()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Compile CS2 recordings into a model-ready LMDB with MP4 metadata.", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--recdir", required=True, type=Path)
    parser.add_argument("--dbfile", required=True, type=Path)
    parser.add_argument("--outlmdb", required=True, type=Path)
    parser.add_argument("--overwrite", action='store_true')
    parser.add_argument("--overridesql", action='store_true')
    ...
    if args.outlmdb.exists():
        if args.overwrite:
            ...
            LOG.warning(f"Removed existing LMDB: {args.outlmdb}")
        else:
            LOG.critical(f"Output path exists. Use --overwrite.")
            sys.exit(1)
    ...
    for rec in db_recordings:
        ...
        if not rec['is_recorded'] and not args.overridesql:
            LOG.critical(f"Rec for {rec['playername']} round {round_num} not recorded.")
            sys.exit(1)
    ...
    for key, paths in round_team_pov_paths.items():
        if len(paths) != 5:
            LOG.warning(f"Round perspective {key} does not have exactly 5 recordings ({len(paths)} found). It will be skipped in final metadata.")
    ...
    shm_cache = SharedMemory(create=True, size=len(packed_cache))
    ...
    initargs = (shm_cache.name, len(packed_cache), args.outlmdb, lock, gs_dtype, pi_dtype, rounds_info, recordings_map, audio_params)
        ...
        for (round_num, team), pov_paths in sorted(round_team_pov_paths.items()):
            if round_num in rounds_info and len(pov_paths) == 5:
                ...
                if all(sorted_paths):
                    ...
                    LOG.warning(f"Could not form complete 5-player POV list for round {round_num}, team {team} due to name mismatch. Skipping from metadata.")
        
        metadata = {"demoname": demoname, "rounds": rounds_metadata}
        
        try:
            with env.begin(write=True) as txn:
                key = f"{demoname}_INFO".encode('utf-8')
                txn.put(key, json.dumps(metadata, indent=2).encode('utf-8'))
        except lmdb.MapFullError:
            info = env.info()
            stats = env.stat()
            psize = stats['psize']
            current_size = info['map_size']
            needed_size = len(json.dumps(metadata, indent=2).encode('utf-8'))
            new_size = current_size + needed_size + MAP_RESIZE_INCREMENT
            env.set_mapsize(new_size)
            with env.begin(write=True) as txn:
                key = f"{demoname}_INFO".encode('utf-8')
                txn.put(key, json.dumps(metadata, indent=2).encode('utf-8'))
