...
def process_rounds(parser: DemoParser, events: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """
    Replaces rounds.py.
    ...
    if round_starts.empty or round_ends.empty:
        print("Warning: Not enough round start/end events to process rounds.")
        return pd.DataFrame()

    rounds_df = pd.merge(
        round_starts[['tick', 'timelimit']],
    ...
    print(f"Processed {len(rounds_df)} rounds.")
    return rounds_df[['round', 'starttick', 'freezetime_endtick', 'endtick', 't_team', 'ct_team']]


def process_mouse(ticks_df: pd.DataFrame) -> pd.DataFrame:
    """
    Replaces mouse.py.
    ...
    if 'aim_punch_angle' not in ticks_df.columns:
        print("Error: 'aim_punch_angle' column not found in tick data. Skipping mouse processing.")
        return pd.DataFrame()

    mouse_df = ticks_df[['tick', 'player_name', 'pitch', 'yaw', 'aim_punch_angle']].copy()
    ...
    mouse_df = mouse_df.sort_values(by=['player_name', 'tick']).reset_index(drop=True)
    grouped = mouse_df.groupby('player_name')
    ...
    result_df = mouse_df[['tick', 'player_name', 'x', 'y']].copy()
    result_df.dropna(inplace=True)
    
    print(f"Processed {len(result_df)} mouse movement records.")
    return result_df


def process_keyboard_location(ticks_df: pd.DataFrame, events: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """
    Replaces keyboard_location.py.
    Processes player inputs, position, inventory, and other states.
    """
    print("Processing keyboard, location, and player state data...")
    ...
    print(f"Optimized player data: removed {initial_rows - len(player_df)} rows.")
    ...
    if not player_df.empty:
        ...
        print("Warning: Player DataFrame is empty after filtering.")
        return pd.DataFrame()
    ...
    print(f"Processed {len(player_df)} player state records.")
    return player_df


def process_buy_sell_drop(parser: DemoParser, player_df: pd.DataFrame, events: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, Set[Tuple[int, str]]]:
    """
    Replaces buy_sell_drop.py.
    Analyzes events and player states to find buy, sell, and drop actions.
    """
    print("Processing buy, sell, and drop events...")
    ...
    print(f"Processed {len(actions_df)} buy/sell/drop actions.")
    return actions_df, buyzone_presence


def merge_data(player_df: pd.DataFrame, rounds_df: pd.DataFrame, mouse_df: pd.DataFrame, actions_df: pd.DataFrame, buyzone_set: Set[Tuple[int, str]]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Replaces merge.py.
    Merges all processed DataFrames into a final, unified DataFrame.
    """
    print("Merging all data into final structure...")
    ...
    print(f"Filtered player data by round times: removed {initial_rows - len(merged_df)} rows.")
    ...
    print(f"Final merged player table has {len(final_player_df)} rows.")
    return final_player_df, rounds_df


def write_to_db(output_path: str, player_df: pd.DataFrame, rounds_df: pd.DataFrame):
...
def main():
    ...
    out_dir = Path(args.out or f"data_{demo_name}")
    out_dir.mkdir(exist_ok=True)
    db_path = out_dir / "merged.db"

    print(f"Starting processing for demo: {demo_path}")
    print(f"Output will be saved to: {db_path}")

    # --- 1. Single Parse ---
    print("Parsing demo file once... This may take a while.")
    
    # Define all properties and events needed across all original scripts
    tick_props = [
    ...
    rounds_data = process_rounds(parser, events)
    mouse_data = process_mouse(ticks_df)
    player_data = process_keyboard_location(ticks_df, events)
    actions_data, buyzone_data = process_buy_sell_drop(parser, player_data, events)
    
    # --- 3. Merge Data ---
    final_player_data, final_rounds_data = merge_data(player_data, rounds_data, mouse_data, actions_data, buyzone_data)

    # --- 4. Write Final DB ---
    write_to_db(str(db_path), final_player_data, final_rounds_data)
