...
# =============================================================================
# Helper: ignore coaches/observers by activity + name patterns
# =============================================================================
import re as _re

IGNORE_NAME_PATTERNS = _re.compile(r"(coach|observer|spectat|analyst|camera)", _re.I)

def _score_activity(conn: sqlite3.Connection, start_tick: int, end_tick: int, names: list) -> dict:
    """
    Score players by in-round activity to distinguish real players from spectators/coaches.
    Uses the merged 'player' table: keyboard_input, active_weapon, health; optionally RAREACTIONS if present.
    """
    if not names:
        return {}
    ...
    # Per-tick activity from 'player'
    q = f"""
        SELECT playername,
               SUM(CASE WHEN keyboard_input IS NOT NULL AND keyboard_input <> '' AND keyboard_input <> '[]' THEN 1 ELSE 0 END) AS key_ticks,
               SUM(CASE WHEN active_weapon IS NOT NULL AND active_weapon <> '' THEN 1 ELSE 0 END) AS weap_ticks,
               SUM(CASE WHEN health IS NOT NULL AND health > 0 THEN 1 ELSE 0 END) AS alive_ticks
        FROM player
        WHERE tick BETWEEN ? AND ? AND playername IN ({placeholders})
        GROUP BY playername
    """
    try:
        cur.execute(q, (start_tick, end_tick, *names))
        rows = {r[0]: (r[1] or 0, r[2] or 0, r[3] or 0) for r in cur.fetchall()}
    except sqlite3.OperationalError:
        rows = {}

    # Optional: Buy/Drop/Action events from RAREACTIONS if present
    q2 = f"SELECT playername, COUNT(*) FROM RAREACTIONS WHERE tick BETWEEN ? AND ? AND playername IN ({placeholders}) GROUP BY playername"
    act = {}
    try:
        cur.execute(q2, (start_tick, end_tick, *names))
    ...
    scores = {}
    for name in names:
        if IGNORE_NAME_PATTERNS.search(name or ""):
            scores[name] = -1e9  # hard ignore; sink to bottom
            continue
        key_ticks, weap_ticks, alive_ticks = rows.get(name, (0, 0, 0))
        actions = act.get(name, 0)
        # Weighted score: actions strongest; then input/weapon/alive time
        scores[name] = actions*5 + key_ticks*1.5 + weap_ticks*1.0 + alive_ticks*0.5
    return scores

def _filter_to_real_five(conn: sqlite3.Connection, roster: list, start_tick: int, end_tick: int) -> list:
    """
    roster: [[player_name, last_death_tick], ...]
    Return a pruned roster of exactly 5 entries, prioritizing active players in-window.
    """
    if not isinstance(roster, list):
        return []
    names = [p[0] for p in roster if p and isinstance(p, list) and len(p)>0 and isinstance(p[0], str)]
    # Short-circuit: if already 5, still verify no obvious coach labels
    scores = _score_activity(conn, start_tick, end_tick, names)
    # Keep names with positive signal; if >5, take top 5 by score; if <5, take top by score to fill
    keep = [n for n in names if scores.get(n, 0) > 0]
    if len(keep) < 5:
        keep = [n for n, _ in sorted(scores.items(), key=lambda kv: kv[1], reverse=True)[:5]]
    else:
        keep = [n for n, _ in sorted({n: scores[n] for n in keep}.items(), key=lambda kv: kv[1], reverse=True)[:5]]
    # Rebuild roster preserving (name, death_tick) and original order when possible
    pruned = [pair for pair in roster if pair and pair[0] in keep]
    seen = {p[0] for p in pruned}
    for n in keep:
        if n not in seen:
            pruned.append([n, -1])
    return pruned[:5]
...
print("    Cleaning and copying 'rounds' table (dropping coaches/observers)...")
# Fetch all round rows and rewrite team arrays to 5 real players
all_rounds = rounds_conn.cursor().execute("SELECT * FROM ROUNDS").fetchall()
clean_rows = []
for row in all_rounds:
    # Assume last two cols are t_team, ct_team
    row = list(row)
    t_json = row[-2]
    ct_json = row[-1]
    try:
        t_raw = json.loads(t_json or "[]")
    except Exception:
        t_raw = []
    try:
        ct_raw = json.loads(ct_json or "[]")
    except Exception:
        ct_raw = []
    # Indices: 0=round, 1=starttick, 2=freezetime_endtick, 3=endtick (per _rc_fetch_and_process_rounds select)
    starttick = row[1] if len(row) > 1 else None
    freezetime_endtick = row[2] if len(row) > 2 else None
    endtick = row[3] if len(row) > 3 else None
    t_clean = _filter_to_real_five(merged_conn, t_raw, freezetime_endtick or starttick or 0, endtick or (starttick or 0))
    ct_clean = _filter_to_real_five(merged_conn, ct_raw, freezetime_endtick or starttick or 0, endtick or (starttick or 0))
    row[-2] = json.dumps(t_clean)
    row[-1] = json.dumps(ct_clean)
    clean_rows.append(tuple(row))

# Insert cleaned rounds
if clean_rows:
    merged_cursor.executemany("INSERT INTO rounds VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", clean_rows)

# Build definitive keep-set of player names from cleaned rounds to purge 'player' table
keep = set()
for r in clean_rows:
    try:
        keep.update(p[0] for p in json.loads(r[-2]))
        keep.update(p[0] for p in json.loads(r[-1]))
    except Exception:
        pass

if keep:
    ph = ",".join("?" for _ in keep)
    merged_cursor.execute(f"DELETE FROM player WHERE playername NOT IN ({ph})", tuple(keep))
...
def _rc_fetch_and_process_rounds(conn: sqlite3.Connection) -> List[Tuple]:
    ...
    for round_num, starttick, freezetime_endtick, endtick, t_team_json, ct_team_json in all_rounds_data:
        ...
        t_players = _filter_to_real_five(conn, t_players, freezetime_endtick or starttick, endtick)
        ct_players = _filter_to_real_five(conn, ct_players, freezetime_endtick or starttick, endtick)
