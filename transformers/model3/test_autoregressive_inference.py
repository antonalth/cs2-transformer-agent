...
from dataset import DatasetRoot, FRAME_RATE
from lightning_module import CS2PredictorModule


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate model3 autoregressive inference and cache cropping on a real dataset sample."
    )
...
def load_backbone(global_cfg: GlobalConfig, checkpoint: str | None, device: torch.device):
    if checkpoint:
...
def load_sample(global_cfg: GlobalConfig, split: str, sample_index: int, num_frames: int):
    ...
    return sample, split, actual_index
...
def main() -> None:
    ...
    backbone = load_backbone(global_cfg, args.checkpoint, device)

    sample, actual_split, actual_index = load_sample(global_cfg, args.split, args.sample_index, args.num_frames)
    num_frames = min(args.num_frames, sample.images.shape[0])
    if num_frames <= 0:
    ...
    if sample.audio.shape[-1] < total_audio_samples:
        raise RuntimeError(
            f"sample audio is shorter than requested window: have {sample.audio.shape[-1]}, "
            f"need {total_audio_samples}"
        )

    images = sample.images[:num_frames].unsqueeze(0).to(device=device, dtype=backbone.cfg.dtype)
    audio = sample.audio[..., :total_audio_samples].unsqueeze(0).to(device=device, dtype=backbone.cfg.dtype)
    ...
    with torch.inference_mode():
        context_flat, batch_size, time_steps = backbone._prepare_context(images, audio)
        context_view = context_flat.view(
            batch_size,
            time_steps,
            5,
            context_flat.shape[1],
            context_flat.shape[2],
        )

        full_preds = backbone._forward_from_context_flat(context_flat, batch_size, time_steps)

        ar_state = backbone.init_autoregressive_state()
        ar_predictions = []
        for t in range(time_steps):
            step_context = context_view[:, t : t + 1].reshape(batch_size * 5, context_flat.shape[1], context_flat.shape[2])
            pred_t, ar_state = backbone._forward_step_from_context_flat(step_context, batch_size, 1, ar_state)
            ar_predictions.append(pred_t)
        ar_preds = concat_predictions(ar_predictions)
        ...
        print("\nExact cache equivalence (precomputed contexts):")
        for line in exact_lines:
            print(line)

        crop_frames = min(args.crop_frames, time_steps)
        if crop_frames <= 0:
        ...
        for t in range(time_steps):
            step_context = context_view[:, t : t + 1].reshape(batch_size * 5, context_flat.shape[1], context_flat.shape[2])
            pred_t, crop_state = backbone._forward_step_from_context_flat(step_context, batch_size, 1, crop_state)
            ensure_finite(pred_t, f"context_step(frame={t})")

            expected_cached_frames = min(t + 1, crop_frames)
            expected_offset_tokens = max(0, t + 1 - crop_frames) * backbone.cfg.tokens_per_frame
            actual_offsets = []
            for cache in crop_state.split_caches:
                if cache is None:
                    continue
                if not hasattr(cache, "position_offset"):
                    raise RuntimeError("cropped autoregressive cache is missing position_offset")
                actual_offsets.append(int(cache.position_offset))

            offsets_ok = all(offset == expected_offset_tokens for offset in actual_offsets)
            step_ok = crop_state.cached_frames == expected_cached_frames and offsets_ok
            print(
                f"frame={t} cached_frames={crop_state.cached_frames}/{expected_cached_frames} "
                f"total_frames={crop_state.total_frames_processed} "
                f"offset_tokens={actual_offsets[0] if actual_offsets else 0}/{expected_offset_tokens}"
            )
            crop_ok = crop_ok and step_ok

        smoke_state = backbone.init_autoregressive_state(max_cache_frames=crop_frames)
        print("\nEnd-to-end forward_step smoke test:")
        for t in range(time_steps):
            img_step = images[:, t : t + 1]
            audio_step = audio[:, :, :, t * audio_samples_per_frame : (t + 1) * audio_samples_per_frame]
            pred_t, smoke_state = backbone.forward_step(
                img_step,
                audio_step,
                smoke_state,
                max_cache_frames=crop_frames,
            )
            ensure_finite(pred_t, f"forward_step(frame={t})")
            print(
                f"frame={t} cached_frames={smoke_state.cached_frames} "
                f"total_frames={smoke_state.total_frames_processed}"
            )

    if not exact_ok:
        raise SystemExit("exact autoregressive context test failed")
    if not crop_ok:
