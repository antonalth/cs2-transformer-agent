...
EDGE_CORR_V: float = 1.85       # pixel-shift per degree vertically
...
class DefaultBBoxAlg:
    ...
    def _apply_edge_correction(
        ...
        """Shift the bounding-box outwards based on its center angle."""
        ...
        # Compute shifted centre
        new_cx = self.adjust_x(theta_h, cx)
        new_cy = self.adjust_y(theta_v, cy)

        # Determine translation deltas
        dx = new_cx - cx
        dy = new_cy - cy

        # Shift all edges by the same amount
        xmin += dx
        xmax += dx
        ymin += dy
        ymax += dy
    ...
    def adjust_x(self, theta_h: float, x_px: float) -> float:
        """Apply horizontal edge-correction to a given x-coordinate."""
        return x_px + self.k_h * theta_h

    def adjust_y(self, theta_v: float, y_px: float) -> float:
        """Apply vertical edge-correction to a given y-coordinate."""
