...
from ..sim_harness.model4_helpers import observation_to_model4_batch
...
from .checkpoint import LoadedModel4, ensure_model4_import_path, load_model4_checkpoint
...
class Model4InferenceRuntime:
    def __init__(
        self,
        cfg: Model4RuntimeConfig,
        *,
        loaded_model: LoadedModel4 | None = None,
    ) -> None:
        if len(cfg.slot_names) != 1:
            raise ValueError(
                f"model4 runtime supports exactly one controlled slot, got {len(cfg.slot_names)}: {cfg.slot_names}"
            )
        self.cfg = cfg
        self.loaded_model = loaded_model or load_model4_checkpoint(
            cfg.checkpoint_path,
            device=cfg.device,
    ...
    def _cache_stats_locked(self) -> dict[str, Any]:
        tokens_per_frame = int(getattr(self.loaded_model.backbone, "tokens_per_frame", 1))
        cached_frames = int(getattr(self._ar_state, "cached_frames", 0))
        return {
            "tokens_per_frame": tokens_per_frame,
            "cached_tokens": cached_frames * tokens_per_frame,
            "cached_frames": cached_frames,
            "total_frames_processed": int(getattr(self._ar_state, "total_frames_processed", 0)),
            "max_cache_frames": self._ar_state.max_cache_frames,
        }

    def _crop_runtime_cache_locked(self, max_cache_frames: int | None) -> None:
        self._ar_state.max_cache_frames = max_cache_frames
        crop_fn = getattr(self.loaded_model.backbone, "_crop_cache", None)
        if callable(crop_fn):
            crop_fn(self._ar_state)
    ...
    async def connect(self) -> None:
        ...
        async with self._lock:
            ...
            self._loop_task = asyncio.create_task(self._run_loop(), name="model4-runtime-loop")
    ...
    async def set_cache_window_frames(self, cache_window_frames: int | None) -> None:
        async with self._lock:
            ...
            self._crop_runtime_cache_locked(cache_window_frames)
            self._refresh_composite_locked()
    ...
    def _normalize_decoder_calibration_ui_settings(payload: dict[str, Any]) -> DecoderCalibrationSettings:
        ...
        if "keyboard_false_positive_cost_overrides" in payload:
            settings.keyboard_false_positive_cost_overrides = Model4InferenceRuntime._normalize_keyboard_cost_overrides(
                payload,
        ...
        if "keyboard_false_negative_cost_overrides" in payload:
            settings.keyboard_false_negative_cost_overrides = Model4InferenceRuntime._normalize_keyboard_cost_overrides(
                payload,
    ...
    def _panels_for_prediction_locked(self, prediction: dict[str, torch.Tensor], player_index: int) -> list[dict[str, Any]]:
        ...
        for head_name, tensor in prediction.items():
            ...
            if tensor.ndim == 3:
                selected = tensor[0, 0]
            elif tensor.shape[2] == len(self.cfg.slot_names):
    ...
    def _infer_observation(
        ...
        batch = observation_to_model4_batch(
            observation,
            slot_name=self.cfg.slot_names[0],
