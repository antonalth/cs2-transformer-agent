...
class ModelActionDecoder:
    def __init__(self, *, cfg: ActionDecodeConfig, slot_names: list[str] | tuple[str, ...]) -> None:
        ...
        if not self.slot_names:
            raise ValueError("slot_names must contain at least one slot")
    ...
    def decode(
        ...
        *,
        ...
        active_actions, keyboard_trace = self._decode_keyboard(keyboard_logits)
...
def _require_torch():
    if torch is None:
        raise RuntimeError("torch is required for model4 action decoding")
    return torch
