...
class Model3InferenceRuntime:
    ...
    async def _run_loop(self) -> None:
        ...
        try:
            while True:
                ...
                prediction, actions, summaries, inference_ms = await asyncio.to_thread(
                ...
                async with self._lock:
                    ...
                    if count == 1:
                        self._metrics.avg_inference_ms = inference_ms
                    else:
                        prev = self._metrics.avg_inference_ms
                        self._metrics.avg_inference_ms = prev + (inference_ms - prev) / count
    ...
    def _infer_observation(
    ...
    ) -> tuple[dict[str, torch.Tensor], dict[str, list[dict[str, Any]]], dict[str, Any], float]:
        ...
        return prediction_cpu, actions, summaries, inference_ms
